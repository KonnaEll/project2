...
float distance_computation(float** distance, int i, int j, double** curves, double** query_curves, int m, int item)
{
    if(i == 0 && j == 0)
    {
        distance[i][j] = sqrt(pow(curves[item][0] - query_curves[m][0], 2));
    }
    else if(i > 0 && j == 0)
    {
        distance[i][j] = fmax(distance[i-1][j], sqrt(pow(curves[item][i] - query_curves[m][0], 2)));
    }
    else if(i == 0 && j > 0)
    {
        distance[i][j] = fmax(distance[i][j-1], sqrt(pow(curves[item][0] - query_curves[m][j], 2)));
    }
    else if(i > 0 && j > 0)
    {
        distance[i][j] = fmax(fmin(distance[i-1][j],
            fmin(distance[i-1][j-1], distance[i][j-1])),
            sqrt(pow(curves[item][i] - query_curves[m][0], 2)));
    }
...
void lsh_for_frechet(double** vectors, int input_items_counter, char** names, double** query_vectors, int query_items_counter, char** query_names, int dimension, double** curves, double** query_curves, FILE* output_file_ptr, int k, double delta, float* t, int n)
{
    ...
    for(int m=0; m<query_items_counter; m++)    // for every query show the results
    {
        ...
        while(hash_tables[hash_index] != NULL)
        {
            ...
            {
                printf("%d\n", hash_tables[hash_index]->item);
                // calculate distance
                dist = distance_computation(distance, dimension-1, dimension-1, curves, query_curves, m, hash_tables[hash_index]->item);
        ...
        for(int i=0; i<input_items_counter; i++)
        {
            dist = distance_computation(distance, dimension-1, dimension-1, curves, query_curves, m, i);
